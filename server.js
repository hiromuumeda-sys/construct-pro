...
const toYM = (v, dateStr) => {
  if (v) {
    ...
    if (m && dateStr) {
      const y = (String(dateStr).match(/(\d{4})/) || [])[1];
      if (y) return `${y}/${String(m[1]).padStart(2, '0')}`;
    }
  }
...
app.post(
  ...
  h(async (req, res) => {
    ...
    await logAuditReq(req, 'CREATE', 'categories', ins.id, { name, changes: [`新規登録（工事区分: ${name || '-'}）`] });
...
app.put(
  ...
  h(async (req, res) => {
    ...
    await logAuditReq(req, 'UPDATE', 'categories', parseInt(req.params.id), { name: before?.name || name, changes: diffChanges('categories', before, req.body) });
    res.json({ id: req.params.id, ...req.body });
  })
);
...
app.put(
  ...
  h(async (req, res) => {
    ...
    await logAuditReq(req, 'UPDATE', 'orders', parseInt(req.params.id), {
...
app.put(
  ...
  h(async (req, res) => {
    ...
    await logAuditReq(req, 'UPDATE', 'receipts', parseInt(req.params.id), { name: before?.month || month, changes: diffChanges('receipts', before, req.body) });
    res.json({ id: req.params.id, ...req.body });
  })
);
...
app.post(
  ...
  h(async (req, res) => {
    ...
    for (let idx = 0; idx < dataLines.length; idx++) {
      ...
      if (isNaN(amount)) {
        errors.push(`${rowNum}行目: 入金額「${amountStr}」が不正です`);
        continue;
      }
  ...
  h(async (req, res) => {
    ...
    for (const p of projects) {
      ...
      summary.push({
        ...
        completed,
        completed_receivable: completedReceivable,
...
app.post(
  ...
  h(async (req, res) => {
    ...
    await logAuditReq(req, 'CREATE', 'invoices', ins.id, { invoice_no: invoiceNo, project_id: projectId, total });
...
app.get(
  ...
  h(async (req, res) => {
    ...
    if (type === 'sales') {
      ...
      rows.forEach(r => {
        r.month = toYM(r.month, r.received_date);
      });
      csv = toCSV(rows, [
    ...
    await new Promise((resolve, reject) => {
      ...
      points.forEach((p, i) => {
        ...
        doc
          .fillColor('#6b7280')
          .fontSize(6)
          .text(p.ym.replace('-', '/').slice(2), chartX + slot * i, chartY + chartH + 4, { width: slot, align: 'center' });
        doc.fillColor('black');
      });
      // Profit line
      ...
      doc
        .fillColor('#888')
        .fontSize(8)
        .text('※ 売上＝案件の工期開始月の契約金額、利益＝売上−注文確定額', 40, chartY + chartH + 24);
      doc.end();
    });
...
function renderOrderAckSheet(doc, mode, { vendorName, vendorAddr, fields, wareki }) {
  ...
  if (mode === 'order') {
    ...
    y += 13;
    doc.text('なお、工事注文確認の為、注文請書のご返送よろしくお願いいたします。', L, y);
    y += 20;
  } else {
  ...
  y += 24;
  // 日付
  const dateStr = mode === 'order' && wareki ? `令和　${wareki.y}年 ${wareki.m}月　${wareki.d}日` : '令和　　　年　　　月　　　日';
...
function buildDocumentPDF(kind, project, orders) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(8)
      .fillColor('#000')
      .text((isInvoice ? '請求№　' : '見積№　') + no, R - 260, ry, { width: 260, align: 'right' });
    ry += 11;
    if (isInvoice) {
      doc.text('適格請求書発行事業者登録番号　' + COMPANY.regNo, R - 260, ry, { width: 260, align: 'right' });
      ry += 13;
    }
    ...
    y += 15;
    doc.text(isInvoice ? '下記のとおり御請求申し上げます。' : '下記のとおり御見積申し上げます。', L, y);
    y += 20;
    // 合計金額ボックス＋日付
    const boxW = 250,
      boxH = 46;
    ...
    doc
      .fontSize(18)
      .fillColor('#000')
      .text('¥' + total.toLocaleString(), L + 6, y + 20, { width: boxW - 12, align: 'right' });
    ...
    if (!isInvoice) {
      y += 18;
      doc.fillColor(gray).fontSize(8).text('※本見積書の有効期限は発行日より30日間です。', L, y);
    }
    doc.end();
  });
}
...
app.post(
  ...
  h(async (req, res) => {
    ...
    await logAudit(getUserId(req), 'CREATE', 'invitations', inv.id, {
      name: email,
      changes: [`アカウント招待を発行（権限: ${INVITE_ROLES[role]}／24時間有効）`],
    });
...
app.delete(
  ...
  h(async (req, res) => {
    ...
    await logAudit(getUserId(req), 'DELETE', 'invitations', parseInt(req.params.id), {
      name: inv ? inv.email : `#${req.params.id}`,
